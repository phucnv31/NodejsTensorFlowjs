...
var NUM_CLASSES;
var currentModel = createConvModel();
var datas;
var xs;
var ys;
...
function getTrainData() {
    ...
    for (const item of datas) {
        ...
        if (!xs) {
            ...
            // disposed.
            xs = tf.keep(getImgAndResize('E:/AI_ML_DL/Data/vn_celeb_face_recognition/train/' + item.image));
        } else {
            const oldX = xs;
            xs = tf.keep(oldX.concat(getImgAndResize('E:/AI_ML_DL/Data/vn_celeb_face_recognition/train/' + item.image), 0));
            oldX.dispose();
        }
...
function getImgAndResize(path) {
    ...
    // webcam.
    image = tf.image.resizeBilinear(image, [128, 128], false);
